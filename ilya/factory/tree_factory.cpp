...
double create_radius(double length, double radius, double k)
{
    double BE = length - length / k;
    double new_k = length / BE;
    return radius / new_k;
...
void create_main_branches(Binary_Tree *left, Binary_Tree *right, tree_params params, Branch root)
{
    ...
    l_start = params.body.length / params.koef.l_start;
    r_start = params.body.length / params.koef.r_start;
    l_length = params.body.length / params.koef.l_length;
    r_length = params.body.length / params.koef.r_length;
...
void tree_generate(mat &tree, tree_params params, size_t deep, vector<int> &brs_deep, vector<double> &radius)
{
    int i = 0;
    Branch root = create_root(params.body.length, params.body.radius);
...
void nodes_fill(std::shared_ptr<Model> model, mat tree)
{
    for (size_t i = 0; i < size(tree).n_rows; i++)
    {
        ...
        //int b = i * 3 % 256;
        //rgb_color clr = {r,g,b};
        Node node(tree(i,0), tree(i,1), tree(i,2), {0,0,0});
        model->addNode(node);
    }
}
...
size_t calc_deep(double length, double radius, double k)
{
    size_t deep = 0;
    while (length > MIN_BRANCH_LENGTH && radius > MIN_BRANCH_RADIUS)
    {
        radius = create_radius(length, radius, k);
        length /= k;
        deep++;
        //std::cout << radius << " : " << length << endl;
    }
    return deep;
}

size_t calc_tree_deep(tree_params params)
{
    size_t left_deep = calc_deep(params.body.length, params.body.radius, params.koef.l_length);
    size_t right_deep = calc_deep(params.body.length, params.body.radius, params.koef.r_length);
...
std::shared_ptr<Transformed_Model> Honda::buildModel(tree_params params, std::shared_ptr<Transform_Tree> tr_tree)
{
    ...
    size_t deep_tree = calc_tree_deep(params);
    //создаем матрицу преобразований
    ...
    //генерируем дерево
    tree_generate(tree, params, deep_tree, depth, branch_radius);
    ...
    //передаем радиус ствола и количество его граней
