...
std::shared_ptr<Transformed_Model> Cone::buildModel(std::shared_ptr<Model> model, QColor& clr)
{
    ...
    Node node1, node2;
    double radius;
    rgb_color new_color = {clr.red(), clr.green(), clr.blue()};

    //задаем точность разбиения конуса на треугольники (должна быть четной)
    size_t accuracy = size_t(model->getAccuracy());
