...
class NoDrawingTrianglesException : public BaseException
{
private:
    static constexpr const char* ErrorMsg = "Error existence drawing triangles";
public:
    explicit NoDrawingTrianglesException() : BaseException(ErrorMsg) {}
    explicit NoDrawingTrianglesException(const std::string& message) : \
        BaseException(ErrorMsg + message) {}
};
