...
bool check_right_orientation2(Point vec1, Point vec2, Point vec3)
{
    mat m = {
...
void calc_norms(vector<Triangle> &triangles)
{
    ...
    for (size_t i = 0; i < triangles.size(); i++)
    {
        ...
        if (!check_right_orientation2(v_norm, v1, v2))
        {
            v_norm = mult_on_number(v_norm, -1);
        }
...
{
    //x
    coords.push_back({-1, 0, 0});
    coords.push_back({1, 0, 0});
    //y
    coords.push_back({0, -1, 0});
    coords.push_back({0, 1, 0});
    //z
...
void DrawManager::drawWhiteList(BaseDrawer& drw)
{
    drw.draw();
}


void DrawManager::drawcolorfulModel(BaseDrawer& drw, shared_ptr<Object> mod, shared_ptr<Object> camera)
{
    long long i1, i2;
    i1 = __rdtsc();
    shared_ptr<Camera> cm = std::static_pointer_cast<Camera>(camera);

    shared_ptr<Transformed_Model> model = std::static_pointer_cast<Transformed_Model>(mod);
        ...
        ZBuffer buf(700, 700);
        rgb_color clr = {drw.getColor().red(), drw.getColor().green(), drw.getColor().blue()};
        buf.Clear(clr);
        ...
        cout << "Время работы в тиках:" << abs(i2 - i1) << endl;
